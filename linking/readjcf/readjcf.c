    ...
    uint8_t expected_tag)
{
	...
	case JCF_CONSTANT_Class:
		// Print the class.
		struct jcf_cp_class_info *inf = malloc(sizeof(struct jcf_cp_class_info));
		print_jcf_constant(jcf, inf->name_index, JCF_CONSTANT_Utf8);
		break;
	...
	case JCF_CONSTANT_InterfaceMethodref:
		 ...
		 * separated by a '.'.
		 */
		struct jcf_cp_ref_info *ref = malloc(sizeof(struct jcf_cp_ref_info));
		print_jcf_constant(jcf, ref->class_index, JCF_CONSTANT_Utf8);
		printf(".");
		print_jcf_constant(jcf, ref->name_and_type_index, JCF_CONSTANT_Utf8));
		break;
	case JCF_CONSTANT_NameAndType:
		// Print the name and type.
		struct jcf_cp_ref_info *refer = malloc(sizeof(struct jcf_cp_ref_info));
		print_jcf_constant(jcf, refer->name_and_type_index, JCF_CONSTANT_Utf8);
		break;
	case JCF_CONSTANT_Utf8:
		// Print the UTF8.
		struct jcp_cp_utf8_info *info_utf8 = malloc(sizeof(struct jcf_cp_utf8_info) + 4);
		print_jcf_constant(jcf, info_utf8->bytes, JCF_CONSTANT_Utf8);
		break;
	default:
...
process_jcf_header(struct jcf_state *jcf)
{
	...
	// Read the header.
	if (fread(&header.magic, sizeof(uint32_t), 1, jcf->f) != 1)
		return (-1);	
	if (fread(&header.minor_version, sizeof(uint16_t), 1, jcf->f) != 1)
		return (-1);
	if (fread(&header.major_version, sizeof(uint16_t), 1, jcf->f) != 1)
		return (-1);
...
process_jcf_constant_pool(struct jcf_state *jcf)
{
	...
	if (fread(&constant_pool_count, sizeof(uint16_t), 1, jcf->f) != 1)
		return (-1);

	// Allocate the constant pool.
	struct jcf_constant_pool *const_pool = malloc(constant_pool_count * sizeof(struct jcf_cp_info *));
	...
	for (i = 1; i < constant_pool_count; i++) {
		...
		case JCF_CONSTANT_MethodType:
			...
			info_1u2->tag = tag;
			//assign to the i element of the constant pool
			const_pool.pool[i] = info_1u2->tag;
			if(fread(&info_1u2, sizeof(info_1u2), 1, jcf->f) != 1)
				return (-1);
			// call ntohs for the value in fread
		...
		case JCF_CONSTANT_InvokeDynamic:
			...
			info_2u2->tag = tag;
			//assign to the i element of the constant pool
			const_pool.pool[i] = info_2u2->tag;
			if(fread(&info_2u2->body, sizeof(info_2u2->body), 1, jcf->f) < 1)
				return (-1);
			// call ntohs for the two values in fread
		...
		case JCF_CONSTANT_Float:
			...
			info_1u4->tag = tag;
			//assign to the i element of the constant pool
			const_pool.pool[i] = info_1u4->tag;
			if (fread(&info_1u4, sizeof(info_1u4), 1, jcf->f) < 1)
				return (-1);
			// call ntohl for the value in fread
		...
		case JCF_CONSTANT_Double:
			 ...
			 */
			struct jcf_cp_info_2u4 *info_2u4 = malloc(sizeof(struct jcf_cp_info_2u4));
			info_2u4->tag = tag;
			//assign to the i element of the constant pool
			const_pool.pool[i] = info_2u4->tag;
			if (fread(&info_2u4->body, sizeof(info_2u4->body), 1, jcf->f) < 1)
				return (-1);
			// call ntohl for the two values in fread
			info_2u4->body.u4_1 = ntohl(info_2u4->body.u4_1);
			info_2u4->body.u4_2 = ntohl(info_2u4->body.u4_2);
			break;
		case JCF_CONSTANT_Utf8:
			// Read a UTF8 constant.
			struct jcf_cp_utf8_info *info_utf8 = malloc(sizeof(struct jcf_cp_utf8_info) + 4);
			info_utf8->tag = tag;
			//assign to the i element of the constant pool
			const_pool.pool[i] = info_utf8->tag;
			if (fread(&info_utf8, sizeof(info_utf8), 1, jcf->f) < 1)
				return (-1);
			break;
		case JCF_CONSTANT_MethodHandle:
			...
			info_u1u2->tag = tag;
			//assign to the i element of the constant pool
			const_pool.pool[i] = info_u1u2->tag;
			if (fread(&info_u1u2->body, sizeof(info_u1u2->body), 1, jcf->f) < 1)
				return (-1);
			// call ntohs for the u2 value in fread
	 ...
	 */
	 if (jcf->depends_flag) {
			printf("Dependency - ");
			if (print_jcf_constant(jcf, info.class_index, JCF_CONSTANT_Utf8) != 0)
				return (-1);
			print(".");
			if (print_jcf_constant(jcf, info.name_index, JCF_CONSTANT_Utf8) != 0)
				return (-1);
			printf(" ");
			if (print_jcf_constant(jcf, info.descriptor_index, JCF_CONSTANT_Utf8) != 0)
				return (-1);
			printf("\n");
	}
	return (0);
...
process_jcf_interfaces(struct jcf_state *jcf)
{
	...
	if (fread(&interface_count, sizeof(uint16_t), 1, jcf->f) < 1)
		return (-1);
	// Read the interfaces.
	for (i = 0; i < interfaces_count; i++){
		uint16_t interface;
		if (fread(&interface, sizeof(uint16_t), 1, jcf->f) < 1)
			return (-1);
	}
	return (0);
}
...
process_jcf_attributes(struct jcf_state *jcf)
{
	...
	if (fread(&attributes_count, sizeof(uint16_t), 1, jcf->f) != 1)
		return (-1);

	// Read the attributes.
	for (i = 0; i < attributes_count; i++) {
		// Read the attribute name index.
		if (fread(&name_index, sizeof(uint16_t), 1, jcf->f) != 1)
			return (-1);
		// Read the attribute length.
		if (fread(&length, sizeof(uint32_t), 1, jcf->f) != 1)
			return (-1);
		// Read the attribute data.
		for (i = 0; i < length; i++){
			if (fread(&info, sizeof(uint8_t), 1, jcf->f) != 1)
				return (-1);
		}
	}
